...
from .proto import NetworkFormat, Format, Net as pb_Net
...
class Net:
    def __init__(
        ...
        value=NetworkFormat.VALUE_WDL,
        policy=NetworkFormat.POLICY_CONVOLUTION,
    ):
    ...
    def get_weight_amounts(self):
        ...
        if (
            self.pb.format.network_format.network
            == NetworkFormat.NETWORK_SE_WITH_HEADFORMAT
        ):
    ...
    def fill_layer(self, layer, weights):
        ...
        layer.max_val = (
            1 if len(params) == 1 and np.max(params) == 0 else float(np.max(params))
        )
    ...
    def se_ratio(self):
        return (
            np.frombuffer(self.pb.weights.residual[0].se.b1.params, dtype=np.uint16)
            .astype(np.float32)
            .shape[0]
        )
