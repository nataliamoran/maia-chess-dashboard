# The code in `_leela_board.py` and `_uci_to_idx.py` is from [lczero_tools](https://github.com/so-much-meta/lczero_tools) and is used under the GPL license.
...
time_plane = np.ones((8, 8), dtype=np.float32)
...
class LeelaBoard:
    ...
    def __init__(self, *args, **kwargs):
        ...
        if len(args) == 1 and isinstance(args[0], chess.Board):
            self.pc_board = args[0].copy()
        else:
    ...
    def num_pieces(self):
        return len(self.pc_board.piece_map())
    ...
    def lcz_features(self, board_stack_size=8):
        """
        ...
        curdata = self.lcz_stack[-1]
        planes_yielded = 0
        if board_stack_size > 8 or board_stack_size < 1:
            raise AttributeError(
                f"Board histories longer than 8 or shorter than 1 not supported, {board_stack_size} given"
            )
        for data in self.lcz_stack[-1 : -(board_stack_size + 1) : -1]:
            ...
            if not curdata.side_to_move:
                ...
                # We're black
                # planes_tmp = np.unpackbits(memoryview(plane_bytes))[::-1].reshape(12, 8, 8)[::-1]
                # print(planes_tmp)
                # print("==================")
                # planes_tmp = planes_tmp.reshape(2,6,8,8)[::-1,:,::-1]
            ...
            # print([flat_planes[data.repetition]])
            planes_stack.append(planes)
            planes_stack.append([flat_planes[data.repetition]])
            planes_yielded += 13
        empty_planes = [
            flat_planes[0] for _ in range((13 * board_stack_size) - planes_yielded)
        ]
        ...
        planes_stack.append(
            [
                ...
                flat_planes[curdata.side_to_move],
                # flat_planes[0],
                flat_planes[curdata.rule50_count] / 100,
                # time_plane * move_time / max_clock,
                flat_planes[0],
                flat_planes[1],
            ]
        )
    ...
    def decompress_features(cls, compressed_features):
        ...
        result = (
            np.concatenate((piece_plane_arr, scalar_arr))
            .astype(np.float32)
            .reshape(-1, 8, 8)
        )
        return result
